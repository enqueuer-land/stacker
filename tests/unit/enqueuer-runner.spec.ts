...
describe('EnqueuerRunner', () => {
    ...
    it('should spawn enqueuer', () => {
        ...
        expect(spawnMock).toHaveBeenCalledWith('enqueuer', {stdio: ['pipe', 'pipe', 'pipe', 'ipc']});
    });
    ...
    it('should get enqueuer response when iterations matching total iterations', done => {
        ...
        onMessageCallback({event: 'REQUISITION_FINISHED', value: {requisition: {id: 'id', totalIterations: 2, name: 'first'}}});
        onMessageCallback({event: 'REQUISITION_FINISHED', value: {requisition: {id: 'id', totalIterations: 2, name: 'second'}}});
    });
});
